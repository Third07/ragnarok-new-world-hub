(() => {
  ...
  function activeState() {
    return state.summary ? categoryState(state.summary.categories.find((item) => item.id === state.activeId)) : null;
  }
  ...
  function render() {
    ...
    if (current.loading) {
      elements.status.textContent = state.searching
        ? `Searching all ${current.category.label.toLowerCase()}…`
        : `Loading ${current.category.label.toLowerCase()}…`;
  ...
  async function loadNextChunk(current) {
    if (current.loading || current.loadedChunks >= current.category.manifests.length) return;
  ...
  async function startLibrary() {
    if (state.started || !state.summary) return;
    state.started = true;
